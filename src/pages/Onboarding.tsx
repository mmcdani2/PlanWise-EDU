                ...
                <AnimatePresence mode="wait">
                    {step === 0 && (
                        <motion.div key="step1" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4 }} className="space-y-6">
                            ...
                            <div className="flex justify-end">
                                <button disabled={!role} onClick={next} className="mt-4 bg-blue-600 hover:bg-blue-700 px-5 py-2 rounded text-white disabled:opacity-40">Next</button>
                            </div>
                        </motion.div>
                    )}
                    ...
                    {step === 1 && (
                        <motion.div key="step2" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4 }} className="space-y-6">
                            ...
                            <div className="flex justify-between">
                                <button onClick={back} className="text-sm text-blue-300 hover:underline">Back</button>
                                <button onClick={next} disabled={selectedGrades.length === 0} className="bg-blue-600 hover:bg-blue-700 px-5 py-2 rounded text-white disabled:opacity-40">Next</button>
                            </div>
                        </motion.div>
                    )}
                    ...
                    {step === 2 && (
                        <motion.div key="step3" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4 }} className="space-y-6">
                            ...
                            <div className="flex justify-between">
                                <button onClick={back} className="text-sm text-blue-300 hover:underline">Back</button>
                                <button onClick={next} disabled={selectedSubjects.length === 0} className="bg-blue-600 hover:bg-blue-700 px-5 py-2 rounded text-white disabled:opacity-40">Next</button>
                            </div>
                        </motion.div>
                    )}
                    ...
                    {step === 3 && (
                        <motion.div key="step4" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4 }} className="space-y-6">
                            ...
                            <div className="flex justify-between">
                                <button onClick={back} className="text-sm text-blue-300 hover:underline">Back</button>
                                <button onClick={next} disabled={!schoolName.trim()} className="bg-blue-600 hover:bg-blue-700 px-5 py-2 rounded text-white disabled:opacity-40">Next</button>
                            </div>
                        </motion.div>
                    )}
                    ...
                    {step === 4 && (
                        <motion.div key="step5" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4 }} className="space-y-6">
                            ...
                            <div className="flex justify-between">
                                <button onClick={back} className="text-sm text-blue-300 hover:underline">Back</button>
                                <button onClick={next} disabled={!scheduleType} className="bg-blue-600 hover:bg-blue-700 px-5 py-2 rounded text-white disabled:opacity-40">Finish</button>
                            </div>
                        </motion.div>
                    )}
                    ...
                    {step === 5 && (
                        <motion.div key="step6" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4 }} className="space-y-6 text-center">
                            ...
                            <button onClick={() => navigate('/dashboard')} className="mt-6 bg-blue-600 hover:bg-blue-700 px-6 py-3 rounded text-white font-semibold">Go to Dashboard</button>
                        </motion.div>
                    )}

                    {(step < 0 || step > 5) && (
                        <motion.div key="fallback" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center text-white">
                            <p>Something went wrong. Resetting onboarding...</p>
                        </motion.div>
                    )}
