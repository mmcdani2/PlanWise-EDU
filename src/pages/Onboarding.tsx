...
export default function Onboarding() {
  ...
  const handleSubmit = async () => {
    ...
    if (userError || !user) return alert('User session error. Please log in again.');
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#0F172A] via-[#1E293B] to-[#0F172A] px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-xl bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl shadow-2xl px-8 py-10 text-white relative z-10">
        <div className="text-sm mb-6 text-center text-white/70">Step {step + 1} of {totalSteps + 1}</div>
        <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <select
                  ...
                  className="w-full px-4 py-3 rounded-md bg-white text-gray-900 dark:bg-gray-800 dark:text-white border border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
                ...
                <input
                  ...
                  className="p-2 rounded-lg bg-white/10 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                <input
                  type="text"
              ...
              <div className="flex justify-end">
                <button
                  onClick={() => setStep(1)}
                  disabled={!firstName || !lastName || !prefix}
                  className="mt-4 bg-blue-600 hover:bg-blue-700 px-5 py-2 rounded text-white disabled:opacity-40"
                >
                  Next
                </button>
              </div>
          ...
          {step === 6 && (
            <motion.div key="step6" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.4 }} className="space-y-6 text-center">
              ...
              <p className="text-white/80">Welcome to PlanWise, {prefix} {lastName} from {schoolName}.</p>
